...
#'
#' @seealso [query_species()], [plot_sra_availability()]
#'
#' @examples
#' \dontrun{
#' RESULTS <- query_species(c('Vigna angularis', 'Vigna vexillata'))
...
#'
#' Collapses record-level BioSample tissue metadata into species-level anatomy
#' class, subclass, and term counts assigned using the internal ontology.
#'
#' By default, the output includes class-level counts for the major anatomy
#' categories (aerial, ground, reproductive, whole, in_vitro, other, mixed,
#' unknown), plus total BioSample records and operable BioSamples per species.
#' Here, `whole` is reserved for whole-plant or whole-seedling samples, not
#' for whole sub-anatomy such as whole leaves, roots, grains, or spikelets.
#'
#' A BioSample is operable when it contains an accepted sample-source
#' attribute, regardless of whether the value can be assigned to a curated
#' anatomy term. Accepted but missing-like values and accepted values with no
#' ontological match are retained as `unknown`; broad, generic, cross-cutting,
#' or subcellular anatomy values are retained as `other`.
#'
#' At class level, `mixed` is assigned when more than one anatomy class is
#' recovered from accepted fields within the same BioSample. Multi-subclass
#' records within a single class remain assigned to that class in this summary;
#' subclass-level mixed states are handled when `summarise_interaction()` is
#' called with `level = 'anatomy_subclass'`.
#'
#' Profile cache (used by [summarise_interaction()]):
#' In addition to the summary table, this function attaches cached
#' BioSample-level profiles as attributes `biosample_anatomy_profile` and
#' `biosample_canonical_profile`. The anatomy profile contains BioSample-level
#' class and subclass profiles (`anatomy_class` and `anatomy_subclass`), while
#' the canonical profile contains row-level anatomy terms, subclasses, classes,
#' ranks, and ontology fields.
#' These caches are intended to be re-used locally for downstream summaries
#' that require within-species BioSample anatomy structure without re-querying
#' NCBI. In particular, [summarise_interaction()] consumes these caches from
...
#'
#' @return A tibble with one row per species containing class-level counts,
#' total BioSample records, and operable BioSample counts. When `all = TRUE`,
#' canonical anatomy-term counts are included. The tibble has class `gdt_tbl`
#' and carries a `query_info` attribute. It also carries cached BioSample-level
#' profiles as attributes `biosample_anatomy_profile` and
#' `biosample_canonical_profile`, plus metadata in
...
summarise_biosample_availability <- function(results, species = NULL, all = FALSE) {
  ...
  canonical_fields <- c('species', 'biosample_id', 'anatomy_term', 'anatomy_class', 'anatomy_subclass', 'rank', 'ontology_namespace', 'ontology_id', 'ontology_label')
  SPECIES_ALL <- names(results)
  ...
  if (!length(SPECIES_USE)) {
    ...
    attr(OUT, 'biosample_anatomy_profile_info') <- list(
      cached_at_utc = format(as.POSIXct(Sys.time(), tz = 'UTC'), '%Y-%m-%dT%H:%M:%SZ'),
      id_col = 'biosample_id',
      fields = c('species', 'biosample_id', 'anatomy_class', 'anatomy_subclass'),
  ...
  PROGRESS <- .pb_state(.biosample_progress_total(results[SPECIES_USE], post_steps = 4L), enabled = interactive())
  if (!is.null(PROGRESS)) on.exit(.pb_close_state(PROGRESS), add = TRUE)
  META <- .biosample_metadata_core(results = results, species = SPECIES_USE, progress = FALSE, progress_state = PROGRESS, include_empty_records = FALSE)
...
#'
#' Retrieves record-level BioSample tissue metadata for one or more species,
#' then assigns curated anatomy classes, subclasses, and canonical anatomy
#' terms.
#'
#' Optional filters restrict the output to particular anatomy classes,
#' subclasses, or canonical anatomy terms.
#'
#' Accepted sample-source attributes with missing-like values, and accepted
#' values with no curated anatomy match, are retained as `unknown`. Broad,
#' generic, cross-cutting, or subcellular anatomy values that match the curated
#' residual terms are retained as `other`.
#'
#' @details
...
#' follow the curated
#' ontology used by [summarise_biosample_availability()].
#'
#' Unlike [summarise_biosample_availability()], this function returns
#' term-level anatomy assignments and does not replace row-level assignments
#' with `mixed`. Consequently, a BioSample may appear in multiple rows when
#' multiple anatomy terms are detected. The columns `anatomy_class_profile` and
#' `anatomy_subclass_profile` store the BioSample-level collapsed states, where
#' `mixed` means multi-class or multi-subclass respectively. This preserves the
#' recovered tissue assignments while allowing users to recover the accessions
#' that contributed to collapsed summary categories.
#'
...
#' retain.
#' @param anatomy_term Optional character vector of anatomy terms to retain.
#'
#' @return A tibble with one row per BioSample anatomy term, including
#' identifiers, raw and normalised tissue fields, row-level ontology
#' assignments, and BioSample-level `anatomy_class_profile` and
...
anatomy_term     = NULL) {
  ...
  build_bioproject_map <- function(meta) {
    records <- attr(meta, 'biosample_records', exact = TRUE)
    if (is.null(records) || !is.data.frame(records) || !nrow(records)) {
    ...
    if (!'bioproject' %in% names(records)) records$bioproject <- NA_character_
    records |>
      dplyr::mutate(
        biosample = .biosample_resolve_key(.data$biosample_id, .data$input_id),
  ...
  PROGRESS <- .pb_state(.biosample_progress_total(results[SPECIES_USE], post_steps = 3L), enabled = interactive())
  if (!is.null(PROGRESS)) on.exit(.pb_close_state(PROGRESS), add = TRUE)
  META <- .biosample_metadata_core(results = results, species = species, progress = FALSE, progress_state = PROGRESS, keep_xml = FALSE, keep_bioproject = TRUE, include_empty_records = FALSE)
  ...
  BIOPROJECT <- build_bioproject_map(META)
  .pb_advance(PROGRESS)
  OUT <- HITS |>
    dplyr::left_join(
      BIOPROJECT,
      by = c('species', 'input_id', 'entrez_uid', 'biosample')
    ) |>
    dplyr::left_join(
      PROFILE,
...
#' [summarise_biosample_availability()]. BioSamples are linked by shared
...
#' BioSamples in each modality-by-anatomy cell. Expected counts are computed
#' from the species-level marginal totals as
#' `expected = row_total * col_total / total`, where `row_total` is the total
#' number of linked BioSamples in a modality class, `col_total` is the total
#' number of linked BioSamples in an anatomy category, and `total` is the total
#' number of linked BioSamples for that species.
#'
